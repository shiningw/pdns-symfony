+(function ($) {
  $.pdns = {
    ...
    createForm: function (element, options) {
      ...
      var Constructor = $.pdns.formstyle[mode];
  ...
  $.pdns.ajax = {
    post: function () {
      ...
      var resHandler = $.proxy(function (data, textStatus, xhr) {
        if (data.code >= 200 && data.code <= 204) {
          ...
          if (
            ...
            typeof this.successHandler == "function"
          ) {
            this.successHandler();
          }
        } else {
      ...
      var errorHandler = $.proxy(function (data, textStatus, xhr) {
        $.pdns.message.show(
          ".pdns-status-message",
          data.statusText,
          "alert-danger"
        ...
        if (
          ...
          typeof this.errorHandler == "function"
        )
          this.errorHandler();
      }, this);
...
+(function ($) {
  ...
  var Tablerow = function (element, options) {
    ...
    this.$newrowTpl =
      $newrowTpl.length > 0 ? $newrowTpl : $(this.options.newrowTpl);
    this.ajaxurl =
      this.options.ajaxurl || $.pdns.getUrlPrefix() + "/pdns/record/create";
    this.init();
  };
  Tablerow.prototype = {
    ...
    init: function () {
      ...
      this.$element.on(
        "saved.popupform saved.tablerow.linline",
        saveHandler.bind(this)
      );
    },
    render: function () {
      this.$container = this.element.className
        ? $("." + this.container)
        : $("#" + this.container);
...
<div class="popup-container alert" >\
  ...
  $.extend($.pdns.formstyle, { popup: PopupForm });
})(jQuery);
+(function ($) {
  ...
  InlineForm.prototype = {
    ...
    init: function () {
      this.$element.on(
        "click.tablerow.cancel",
        this.options.cancelTarget,
  ...
  $.fn.inlineform.defaults.cancelButton =
    '<button type="button"  class="cancel-button btn btn-default"><span class="glyphicon glyphicon-trash"></span>Cancel</button>';
  //Dnsrecord.newrowTpl = newrowTpl.replace('subdomain name', pdns.getZoneId());
  $.fn.inlineform.defaults.saveButton =
    '<button type="button"  class="save-button btn btn-default"><span class="glyphicon glyphicon-save"></span>Save</button>';
  $.fn.inlineform.defaults.loading =
  ...
  $.extend($.pdns.formstyle, { inline: InlineForm });
})(jQuery);
...
+(function ($) {
  ...
  PdnsConfirm.prototype = {
    init: function () {
      ...
      this.$element.on(
        "click.pdnsconfirm.popup",
        this.selector,
        $.proxy(this.popup, this)
      );
    ...
    popup: function (e) {
      this.render(e);
      this.$nobtn.on(
        "click.pdnconfirm.cancel",
        $.proxy(this.removePopup, this)
      );
    },
    render: function (e) {
      //console.log(e);
